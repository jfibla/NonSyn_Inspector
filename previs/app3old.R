...
#                 # Acepta el nombre de la clase ("search_dbNSFP50a") o una ruta al .class
...
#                 
#               #  tags$hr(),
#               #  h4("Notificación al finalizar (opcional)"),
#               #  checkboxInput("email_notify", "Enviar email al finalizar", FALSE),
#               #  textInput("notify_email", "Tu email", value = ""),
#               #  helpText("Configura SMTP en variables de entorno: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_SENDER."),
#                 
#                 tags$hr(),
...
#                 tags$hr(),
#                 h4("Preview normalizado (100 primeras filas)"),
#                # DTOutput("dbnsfp_norm_tbl"),
#                tags$hr(),
#                h3("Etapa 4 · Visualización (dbNSFP normalizado)"),
#               # Se muestra solo si hay normalizado (se controla desde server via output$viz_ready)
...
ui <- navbarPage(
  ...
  tabPanel(
    "Análisis (Etapas 1–3)",
    # ⬇️ tu UI existente de etapas 1–3 tal cual
    fluidPage(
      sidebarPanel(width = 4,
                   h3("Etapa 1 · Lectura & selección"),
                   fileInput("file", "Carga GWAS (TSV/CSV)", accept = c(".tsv",".txt",".csv")),
      ...
      fluidPage(
        sidebarLayout(
          sidebarPanel(width = 4,
                       ...
                       tags$hr(),
                       # 🔽🔽🔽 AÑADE AQUÍ LOS DOS CONTROLES DEL HEATMAP 🔽🔽🔽
          ...
          mainPanel(
            uiOutput("viz_tabs")      # tus pestañas (Heatmap / GO / KEGG)
          )
        )
      )
    )
...
server <- function(input, output, session){
  ...
  # --- actualizar selector de cromosoma al cargar/alternar dataset ---
  observeEvent(dbnsfp_norm_df(), {
    ...
    if (!is.null(chr_col) && chr_col %in% names(df)) {
      ...
      if ("hm_chr" %in% names(input)) {
        updateSelectInput(session, "hm_chr", choices = choices, selected = "all")
      }
    } else {
      if ("hm_chr" %in% names(input)) {
  ...
  output$heatmap_plot <- renderPlot({
    ...
    id_cols <- base::intersect(c(snp_col, chr_col, bp_col, "ref", "alt", "genename"), nms)
    
    # ⛑️ quitar duplicados y colisiones
    metric_cols <- setdiff(unique(metric_cols), id_cols)
    cols_to_take <- c(id_cols, metric_cols)
    cols_to_take <- cols_to_take[!duplicated(cols_to_take)]
    
    validate(need(length(metric_cols) >= 2, "Elige al menos 2 columnas para el heatmap."))
    
    df_hm <- df[, cols_to_take, drop = FALSE]
    
    # renombrar SOLO las métricas
    ...
    # numéricos
    df_hm[metric_cols_renamed] <- lapply(df_hm[metric_cols_renamed], function(x) suppressWarnings(as.numeric(as.character(x))))
    
    score_matrix <- as.matrix(df_hm[, metric_cols_renamed, drop = FALSE])
    rownames(score_matrix) <- df_hm[[snp_col]]
  ...
  # tabla con los valores (primeras 200 filas)
  output$heatmap_table <- DT::renderDT({
    ...
    snp_col <- session$userData$snp_col
    validate(need(!is.null(snp_col) && snp_col %in% names(df),
                  "No se encontró columna identificadora (SNP/rsID)."))
    
    # mismas columnas que el heatmap
    cols_sel <- if (identical(input$hm_mode, "Score")) input$hm_scores else input$hm_ranks
    if (length(cols_sel) < 2) {
      num_ok <- names(df)[vapply(df, is.numeric, logical(1))]
      num_ok <- base::setdiff(num_ok, c("chr","CHR","BP","POS","Position","hg19_pos_1_based","hg38_pos_1_based"))
    ...
    validate(need(length(cols_sel) >= 2, ""))
    
    # filtro chr
    chr_col <- session$userData$chr_col
    if (!is.null(chr_col) && chr_col %in% names(df) && !is.null(input$hm_chr) && input$hm_chr != "all") {
      key <- suppressWarnings(as.numeric(input$hm_chr))
      key <- if (!is.na(key)) key else input$hm_chr
      df <- df[df[[chr_col]] == key, , drop = FALSE]
    }
    
    df_show <- df %>% dplyr::select(dplyr::all_of(c(snp_col, cols_sel)))
    df_show <- utils::head(df_show, 200)
    DT::datatable(df_show, options = list(pageLength = 10, scrollX = TRUE))
  })
  ...
  output$go_bar <- renderPlot({
    ...
    validate(need(length(go_cols) >= 1,
                  "No se detectaron columnas GO en el archivo normalizado."))
    
    # Umbral FDR del slider (seguro)
    pcut <- if (!is.null(input$go_kegg_pcut) && length(input$go_kegg_pcut) == 1) input$go_kegg_pcut else 0.05
    
    # Extraer términos GO presentes
    df_go <- df[, go_cols, drop = FALSE]
    df_go <- df_go %>% tidyr::pivot_longer(cols = everything(), names_to = "ontology", values_to = "term")
    df_go <- df_go %>% dplyr::filter(!is.na(term) & term != "")
    
    validate(need(nrow(df_go) > 0, "No hay términos GO disponibles."))
    
    # Contar frecuencia de términos por ontología
    go_counts <- df_go %>%
      dplyr::group_by(ontology, term) %>%
      dplyr::summarise(count = dplyr::n(), .groups = "drop")
    
    # Ajuste FDR simulado (si no hay p-valores en el dataset)
    go_counts <- go_counts %>%
      dplyr::group_by(ontology) %>%
      dplyr::mutate(pval = 1 / rank(-count, ties.method = "first"),
                    padj = p.adjust(pval, method = "BH")) %>%
      dplyr::ungroup()
    
    # Filtrar por FDR y top 10 por ontología
    go_top <- go_counts %>%
    ...
    validate(need(nrow(go_top) > 0, "No hay términos GO significativos bajo el umbral FDR."))
    
    # Formatear nombres legibles
    go_top$ontology <- dplyr::recode(go_top$ontology,
                                     "GO_biological_process"   = "BP",
                                     "GO_molecular_function"   = "MF",
                                     "GO_cellular_component"   = "CC")
    
    # Gráfico de barras con ggplot2
    ggplot(go_top, aes(x = stats::reorder(term, count), y = count, fill = ontology)) +
      geom_col() +
      coord_flip() +
      facet_wrap(~ontology, scales = "free_y") +
      labs(x = NULL, y = "Recuento de genes", title = "Términos GO enriquecidos (FDR ≤ umbral)") +
      theme_minimal(base_size = 12) +
  ...
  output$kegg_bar <- renderPlot({
    ek <- kegg_result(); validate(need(!is.null(ek) && nrow(ek@result) > 0, "Sin resultados KEGG."))
    df <- ek@result
    # FDR
    df <- df %>% dplyr::mutate(p.adjust = p.adjust(pvalue, method = "BH")) %>%
